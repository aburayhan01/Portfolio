# 👨‍💻 Abu Rayhan – QA Engineer Portfolio

Welcome to my professional portfolio repository.
This portfolio showcases my background, technical skills, and experience in **Software Quality Assurance (QA), API Testing, and Web Development**.

-----------------------

## 🌐 Live Portfolio

You can view my portfolio here:

🔗 **Portfolio Website:** *(Add your deployed link here if you host it on GitHub Pages or Netlify)*

------------------------

## 👨‍💼 About Me

I am a graduate in **Information and Communication Engineering** from **Noakhali Science and Technology University**.

I have experience in **Software Testing, API Testing, and Web Development**.
My focus is on ensuring software quality through structured testing, bug reporting, and automation tools.

Previously, I worked with **PHP and Laravel** to develop web applications.
Currently, I am focusing on **Software Quality Assurance (QA)** and learning **automation testing using Python**.

---

## 🛠 Skills

### Software Testing

* Manual Testing
* Test Case Design
* API Testing
* Bug Reporting
* Regression Testing

### Tools & Technologies

* Postman
* Newman
* Git & GitHub
* REST API
* JSON

### Programming

* C / C++
* Python
* JavaScript
* C#
* PHP
* SQL

---

## 💼 Experience

### Web Development & QA Intern

**Business Automation**
📅 Jun 2024 – Sep 2024

* Developed scalable web applications using **PHP & Laravel**
* Designed and optimized **MySQL databases**
* Performed **API testing using Postman**
* Used **Git for version control and collaboration**
* Followed **SDLC and SRS documentation**

---

### Network Engineering Intern

**Bangladesh Telecommunications Company Limited (BTCL)**
📅 Nov 2024

* Configured and monitored **LAN/WAN networks**
* Worked with **PSTN and VoIP communication systems**
* Performed **optical fiber splicing**
* Conducted **OTDR testing for network diagnostics**

---

## 📚 Education

**B.Sc. in Information and Communication Engineering**
Noakhali Science and Technology University
CGPA: **3.11**

---

## 🧠 Competitive Programming

* Solved **50+ problems on URI Online Judge**
* Solved **30+ problems on LeetCode**
* Strong foundation in **problem solving and algorithms**

---

## 📬 Contact

📧 Email: *(aburayahanmondal214@gmail.com)*
💼 LinkedIn: *([](https://www.linkedin.com/in/aburayhan))*
🐙 GitHub: https://github.com/aburayhan
🌐 Portfolio: *(link)*

---

⭐ If you like this portfolio, feel free to **star the repository**.
